...
pub const ATOMIC_PROCEDURES: &[ProcedureSignature] = &[
    // apoc.atomic.add(node NODE, property STRING, value INTEGER)
    ("apoc.atomic.add", vec![
        ("node", ApocType::Node),
    ...
    // apoc.atomic.insert(node NODE, property STRING, index INTEGER, value ANY)
    ...
    // apoc.atomic.subtract(node NODE, property STRING, value INTEGER)
...
mod tests {
    ...
    fn test_atomic_insert_signature() {
        ...
        assert_eq!(procedure.1[1].0, "property");
        assert_eq!(procedure.1[1].1, ApocType::String);
        assert_eq!(procedure.1[2].0, "index");
        assert_eq!(procedure.1[2].1, ApocType::Integer);
        assert_eq!(procedure.1[3].0, "value");
        assert_eq!(procedure.1[3].1, ApocType::Any);
    }
    ...
    fn test_all_atomic_procedures_have_signatures() {
        ...
        for (name, args, yields) in ATOMIC_PROCEDURES {
