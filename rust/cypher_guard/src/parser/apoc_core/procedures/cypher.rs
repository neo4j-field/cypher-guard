...
pub static CYPHER_PROCEDURES: LazyLock<Vec<ProcedureSignature>> = LazyLock::new(|| {
    vec![
        // apoc.cypher.doIt(query STRING, params MAP)
        ("apoc.cypher.doIt", vec![
            ("query", ApocType::String),
            ("params", ApocType::Map)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.run(query STRING, params MAP)
        ("apoc.cypher.run", vec![
            ("query", ApocType::String),
            ("params", ApocType::Map)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runMany(queries LIST<STRING>, params LIST<MAP>)
        ("apoc.cypher.runMany", vec![
            ("queries", ApocType::List),
            ("params", ApocType::List)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runManyReadOnly(queries LIST<STRING>, params LIST<MAP>)
        ("apoc.cypher.runManyReadOnly", vec![
            ("queries", ApocType::List),
            ("params", ApocType::List)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runSchema(query STRING, params MAP)
        ("apoc.cypher.runSchema", vec![
            ("query", ApocType::String),
            ("params", ApocType::Map)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runTimeboxed(query STRING, params MAP, timeout INTEGER)
        ...
        // apoc.cypher.runWrite(query STRING, params MAP)
        ("apoc.cypher.runWrite", vec![
            ("query", ApocType::String),
            ("params", ApocType::Map)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runFirstColumnMany(query STRING, params MAP)
        ("apoc.cypher.runFirstColumnMany", vec![
            ("query", ApocType::String),
            ("params", ApocType::Map)
        ], vec![("result", ApocType::Any)]),
        
        // apoc.cypher.runFirstColumnSingle(query STRING, params MAP)
...
mod tests {
    ...
    fn test_all_cypher_procedures_have_signatures() {
        ...
        for (name, args, yields) in CYPHER_PROCEDURES.iter() {
