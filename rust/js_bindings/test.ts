...
function testValidQueries() {
    ...
    for (const query of validQueries) {
        ...
        if (nativeBinding.hasValidCypher) {
            assert(nativeBinding.hasValidCypher(query, schemaJson), `hasValidCypher should return true: ${query}`);
        }
        
        if (nativeBinding.getValidationErrors) {
            const errors = nativeBinding.getValidationErrors(query, schemaJson);
            assert(errors.length === 0, `Should have no errors for valid query: ${query}`);
        }
    }
    ...
    for (const query of validQPPQueries) {
        ...
        if (nativeBinding.hasValidCypher) {
            assert(nativeBinding.hasValidCypher(query, schemaJson), `QPP query should be valid: ${query}`);
        }
    }
...
function testInvalidQueries() {
    ...
    for (const query of allInvalidQueries) {
        ...
        if (nativeBinding.hasValidCypher) {
            assert(!nativeBinding.hasValidCypher(query, schemaJson), `hasValidCypher should return false: ${query}`);
        }
        
        // Test structured errors if available
        if (nativeBinding.getStructuredErrors) {
            const structuredErrors = nativeBinding.getStructuredErrors(query, schemaJson);
            assert(structuredErrors.has_errors, `Should have errors in structured format: ${query}`);
            assert(structuredErrors.error_count > 0, `Error count should be > 0: ${query}`);
            assert(structuredErrors.suggestions.length > 0, `Should have suggestions: ${query}`);
        }
    }
...
function testSchemaFunctions() {
    ...
    if (nativeBinding.dbSchemaRelationshipPatternNew) {
        const relationship = nativeBinding.dbSchemaRelationshipPatternNew('Person', 'Movie', 'ACTED_IN');
        ...
        assert(relationship.rel_type === 'ACTED_IN', 'Relationship type should match');
    ...
    if (nativeBinding.dbSchemaConstraintNew) {
        const constraint = nativeBinding.dbSchemaConstraintNew(
            1, 'unique_person_name', 'UNIQUE', 'NODE', ['Person'], ['name']
        );
        assert(constraint, 'Should create constraint');
        assert(constraint.id === 1, 'Constraint ID should match');
        assert(constraint.name === 'unique_person_name', 'Constraint name should match');
        assert(constraint.constraint_type === 'UNIQUE', 'Constraint type should match');
        ...
        assert(Array.isArray(constraint.properties), 'Should have properties array');
        console.log('✅ Constraint creation test passed!');
    }
    
    if (nativeBinding.dbSchemaIndexNew) {
        const index = nativeBinding.dbSchemaIndexNew('Person', ['name'], 1000, 'BTREE');
...
function testStructuredErrors() {
    ...
    if (nativeBinding.getStructuredErrors) {
        ...
        const structuredErrors = nativeBinding.getStructuredErrors(invalidQuery, schemaJson);
        ...
        const hasSchemaErrors = structuredErrors.categories.schema_errors.length > 0;
        const hasPropertyErrors = structuredErrors.categories.property_errors.length > 0;
        const hasSyntaxErrors = structuredErrors.categories.syntax_errors.length > 0;
        const hasTypeErrors = structuredErrors.categories.type_errors.length > 0;
        
        assert(hasSchemaErrors || hasPropertyErrors || hasSyntaxErrors || hasTypeErrors, 
